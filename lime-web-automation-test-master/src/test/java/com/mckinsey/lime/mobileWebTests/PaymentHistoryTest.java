...
@Deprecated
public class PaymentHistoryTest extends MobileWebBaseTest {
    ...
    @Test(groups = {"mobileWeb"}, description = "Payment History Test")
    public void testPaymentHistory(ITestContext context) throws IOException {
        ...
        PaymentHistoryPage paymentHistoryPage = new PaymentHistoryPage(getDriver());
        ...
        sideMenu.navigate2PaymentHistory();

        paymentHistoryPage.clickDropDown();

        final List<String> productsInDropDown = paymentHistoryPage.getProductsInDropDown();


        final List<String> expected = customResponse_Products.getResponseObj().getProductsForPaymentHistory().stream()
        ...
        if (expected.size() == productsInDropDown.size()) {
            ...
            CustomAssertions.assertTrue(customClassObj, false,
                    "Expected And Actual entries in the dropdown are not same \n \t Expected: " + expected + "\n\t Actual: " + productsInDropDown);
        }
        landingPage.logOutFromApp();
    }
}
